...
本文档说明 Mobile Chat 如何将旧版和 Agent/Clawbot SSE 事件转换为统一的过程状态。`src/views/chat/index.vue`、`src/views/pc/index.vue` 共用同一个 Store 和消息组件，机器人文本消息均通过 `ProcessTimeline` 展示过程。
...
- `src/views/chat/index.vue`：Mobile 页面编排、发送、停止和滚动，不解析 SSE。
- `src/views/pc/index.vue`：Mobile 项目的 `/pc` 页面，复用相同 Store 和消息组件。
...
- `views/chat` 与 `views/pc` 的机器人文本消息统一使用过程时间线。
...
- 时间线样式仅存在于消息组件；`views/pc` 的宽度、边距、头像、头部和输入区布局不修改。
