...
<template>
  <a-modal
    width="700px"
    v-model:open="show"
    :title="`${formState.id ? '编辑' : '创建'}Messenger接入渠道`"
    @cancel="handleCancel"
  >
    <template #footer>
    ...
    <div class="add-wechat-app-alert" v-if="step === 1">
      <a-alert class="zm-alert-info" type="info" show-icon>
        <template #message>
          <div>请按照 <a href="https://help.chatwiki.com/zh/docs/configuring-messenger" target="_blank">配置步骤</a>配置完成后，在Facebook开发者后台获取一下信息。</div>
        </template>
      </a-alert>
      ...
      <div class="alert-body">
        <div class="form-box">
          <a-form ref="formRef" layout="vertical" :model="formState" :rules="formRules">
            ...
            <a-form-item label="主页ID" name="app_id">
              <a-input v-model:value.trim="formState.app_id" placeholder="请输入主页ID" />
            </a-form-item>
            <a-form-item label="主页口令" name="app_secret">
              <a-input v-model:value.trim="formState.app_secret" placeholder="请输入主页口令" />
            </a-form-item>
          </a-form>
        </div>
      </div>
    ...
    <div class="add-wechat-app-alert" v-if="step === 2">
      ...
      <div class="config-preview-box">
        <!-- <img class="config-preview-img" src="@/assets/img/robot/ding-example02.png" /> -->
      </div>
    </div>
  </a-modal>
</template>
...
const defaultAvatar = '/upload/default/facebook-messenger.png'
