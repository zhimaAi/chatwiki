...
<template>
  <div class="">
    <a-alert
      class="tip-alert"
      :message="t('tip_bind_wecom')"
      type="info"
      show-icon
    />
    <div class="wechat-app-list">
      <AddWechatApp :label="t('label_bind_wecom')" @click="showAddAlert" />
      <WechatAppItem
        v-for="item in list"
        :key="item.id"
        :item="item"
    ...
    <AddWeComRobotAlert ref="addAppAlertRef" @ok="onSaveSuccess" />
  </div>
</template>
...
import AddWeComRobotAlert from './add-wecom-robot-alert.vue'

const { t } = useI18n('views.robot.robot-config.external-service.components.wecom-robot')
