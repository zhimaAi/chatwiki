...
<template>
  <div class="">
    <a-alert class="tip-alert" type="info" show-icon>
      <template #message>{{ t('tip_bind_feishu_robot') }}</template>
    </a-alert>
    <div class="wechat-app-list">
      <AddWechatApp :label="t('label_bind_feishu_robot')" @click="showAddAlert" />
      <WechatAppItem
        v-for="item in list"
        :key="item.id"
        :item="item"
    ...
    <AddFeishuAlert ref="addAppAlertRef" @ok="onSaveSuccess" />
  </div>
</template>
...
import AddFeishuAlert from "@/views/robot/robot-config/external-service/components/add-feishu-alert.vue";

const { t } = useI18n('views.robot.robot-config.external-service.components.feishu-robot')
