...
<template>
  <div class="">
    <a-alert class="tip-alert zm-alert-info" type="info" show-icon>
      <template #message>通过阿里云ChatApp接入，接入后，当前机器人可自动回复用户咨询</template>
    </a-alert>
    <div class="wechat-app-list">
      <AddWechatApp label="绑定WhatsApp" @click="showAddAlert" />
      <WechatAppItem
        v-for="item in list"
        :key="item.id"
        :item="item"
    ...
    <AddWhatsApp ref="addAppAlertRef" @ok="onSaveSuccess" />
  </div>
</template>
...
import AddWhatsApp from './add-whats-app.vue'
