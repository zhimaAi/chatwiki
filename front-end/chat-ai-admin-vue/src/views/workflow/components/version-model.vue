...
const getWorkflowLockPayload = inject('getWorkflowLockPayload', () => ({}))
const handleWorkflowLockLost = inject('handleWorkflowLockLost', () => {})
...
const handleOk = () => {
  ...
  workFlowPublishVersion({
    ...
    .catch((res) => {
      if (res?.data?.lock_conflict == 1) {
        handleWorkflowLockLost(res.data)
      }
      if (res.data && res.data.err_node_key) {
        emit('handleOpenErrorNode', res.data.err_node_key)
      }
