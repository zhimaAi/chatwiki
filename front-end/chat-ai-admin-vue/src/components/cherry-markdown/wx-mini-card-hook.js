...
// 自定义段落语法：将 [wx_mini_card]{...json...}[/wx_mini_card] 渲染成小程序卡片占位 HTML
const wxMiniCardHook = CherryEngine.createSyntaxHook('wxMiniCard', CherryEngine.constants.HOOKS_TYPE_LIST.PAR, {
  needCache: true,
  // beforeMakeHtml 先于所有 makeHtml 执行，抢先缓存标记成占位符，
  // 避免结束符 [/wx_mini_card] 被链接等行内语法提前破坏成 <a> 标签
  beforeMakeHtml(str) {
    return str.replace(this.RULE.reg, (whole, json) => {
      try {
        const card = JSON.parse(json)
        const sign = this.$engine.hash(whole)
        const lines = this.getLineCount(whole)
        const enc = (v) => encodeURIComponent(v || '')
        // HTML 转义，用于内容文字
        const esc = (v) => (v || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;')
        // 小程序图标 inline SVG（不依赖 sprite）
