...
import { objToFormData, showErrorMsg, getAdminUserId } from '@/utils'
...
const defaultRequestInterceptors = (config) => {
  if (config.method === 'post') {
    ...
    config.data.admin_user_id = getAdminUserId()
  }

  if (config.method === 'get') {
    config.params = config.params || {}
    config.params.admin_user_id = getAdminUserId()
  }
...
const defaultResponseInterceptors = (response) => {
  ...
  if (response?.config?.responseType === 'blob') {
    ...
    // 是否显示错误提示
    if (!response.config.notDisplayErrorTips) {
