...
export const useChatStore = defineStore('chat', () => {
  ...
  const ensureAvatarShow = (config: ExternalConfigPc, applicationType: any, sourceConfig?: Partial<ExternalConfigPc>) => {
  ...
  const createChat = async (data: Chat) => {
    ...
    try {
      ...
      const currentConfig = getCurrentConfig(robotInfo.multi_lang_configs)
  ...
  const upDataUiStyle = (data) => {
    ...
    ensureAvatarShow(externalConfigPC, robot.application_type, data)
