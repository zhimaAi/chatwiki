...
func (e *ErrorResponse) Error() error {
	...
	if e.HTTPStatusCode > 0 {
		return errors.New(fmt.Sprintf("Request error, status code: %d, business code: %s, message: %s", e.HTTPStatusCode, e.Err.Code, e.Err.Message))
	}
	return errors.New(fmt.Sprintf("Request error, message: %s", e.Err.Message))
}
