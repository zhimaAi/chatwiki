...
func GetNodeByKey(robotId uint, nodeKey string) (NodeAdapter, msql.Params, error) {
	...
	case NodeTypeStart:
		return &StartNode{nextNodeKey: info[`next_node_key`]}, info, nil
	case NodeTypeTerm:
		return &TermNode{params: nodeParams.Term, nextNodeKey: info[`next_node_key`]}, info, nil
	case NodeTypeCate:
		return &CateNode{params: nodeParams.Cate}, info, nil
	case NodeTypeCurl:
	...
	case NodeTypeFinish:
		return &FinishNode{params: nodeParams}, info, nil
	default:
...
func (n *CateNode) Running(flow *WorkFlow) (output common.SimpleFields, nextNodeKey string, err error) {
	flow.Logs(`执行问题分类逻辑...`)
...
func (n *FinishNode) Running(flow *WorkFlow) (output common.SimpleFields, nextNodeKey string, err error) {
	flow.Logs(`执行结束节点逻辑...`)
	flow.isFinish = true
