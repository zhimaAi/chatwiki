...
func LlmLogRequest(
	...
	// record request logs
	_, err = msql.Model(`llm_request_logs`, Postgres).Insert(data)
	if err != nil {
		logs.Error(err.Error())
		return err
	}
...
func statToken(_type string, adminUserId int, corpName string, model string, promptToken int, completionToken int) error {
	...
	stats, err := msql.Model(`llm_token_daily_stats`, Postgres).
	...
	if len(stats) == 0 {
		_, err := msql.Model(`llm_token_daily_stats`, Postgres).Insert(msql.Datas{
	...
	_, err = msql.Model(`llm_token_daily_stats`, Postgres).
		Where(`admin_user_id`, cast.ToString(adminUserId)).
		Where(`corp`, corpName).
		Where(`model`, model).
		Where(`type`, _type).
...
func statDailyActiveUser(adminUserId int, robot msql.Params, appType string) error {
	...
	count, err := msql.Model(`llm_request_logs`, Postgres).
		Where(`admin_user_id`, cast.ToString(adminUserId)).
	...
	row, err := msql.Model(`llm_request_daily_stats`, Postgres).
		Where(`admin_user_id`, cast.ToString(adminUserId)).
	...
	if len(row) == 0 {
		_, err = msql.Model(`llm_request_daily_stats`, Postgres).Insert(msql.Datas{
			`admin_user_id`: adminUserId,
			`robot_id`:      robot[`id`],
			`app_type`:      appType,
			`date`:          time.Now().Format(`2006-01-02`),
		...
		_, err = msql.Model(`llm_request_daily_stats`, Postgres).Where(`id`, row[`id`]).Update(msql.Datas{
			`amount`:      count,
			`update_time`: tool.Time2Int(),
		})
		if err != nil {
...
func statDailyNewUser(adminUserId int, robot msql.Params, appType, openid string) error {
	...
	if exists == 0 {
		rows, err := msql.Model(`llm_request_logs`, Postgres).
			Where(`admin_user_id`, cast.ToString(adminUserId)).
			Where(`source_robot_id`, robot[`id`]).
			Where(`app_type`, appType).
			Where(`create_time`, `>=`, cast.ToString(todayStartTime.Unix())).
		...
		_, err = Redis.SAdd(ctx, key, openidList...).Result()
		if err != nil {
			logs.Error(err.Error())
			return err
		}
		_, err = Redis.Expire(ctx, key, 24*time.Hour).Result()
		if err != nil {
			logs.Error(err.Error())
			return err
		}
	} else {
		_, err = Redis.SAdd(ctx, key, openid).Result()
		if err != nil {
			logs.Error(err.Error())
			return err
		}
	...
	newUserCount, err := Redis.SCard(ctx, key).Result()
	if err != nil {
		logs.Error(err.Error())
		return err
	}
	...
	if len(row) == 0 {
		...
		_, err = msql.Model(`llm_request_daily_stats`, Postgres).
			Where(`admin_user_id`, cast.ToString(adminUserId)).
			Where(`robot_id`, robot[`id`]).
			Where(`app_type`, appType).
			Where(`date`, time.Now().Format(`2006-01-02`)).
