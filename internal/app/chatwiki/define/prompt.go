...
请确保输出的JSON格式正确，只返回JSON数组，不要有其他文字说明。
`
...
const PromptDefaultEntityExtract = `请从下面的问题中提取多组实体关系三元组，以便进行知识图谱检索。
每个三元组包含"主体实体"、"关系"和"客体实体"三部分。
你需要提供至少2个、最多5个不同的三元组组合。尽量从不同角度提取关键概念。
只输出JSON数组格式，每个元素是一个三元组数组，不要有其他文字说明。
...
输出示例：[["实体1", "关系1", "客体1"], ["实体2", "关系2", "客体2"], ["实体3", "关系3", "客体3"], ["实体4", "关系4", "客体4"], ["实体5", "关系5", "客体5"]]`
