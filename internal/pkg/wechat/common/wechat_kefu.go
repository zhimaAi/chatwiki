...
func GetWechatKefuOpenid(adminUserId int, message map[string]any) string {
	...
	if e, o := GetExternalUserInfo(openid); len(e) == 0 || len(o) == 0 {
		...
		_, err := msql.Model(`wechat_kefu_openid_map`, lib_define.Postgres).Insert(data)
		if err != nil {
			logs.Error(err.Error())
		}
		//clear cached data
		lib_redis.DelCacheData(lib_define.Redis, &WechatKefuOpenidCacheBuildHandler{Openid: openid})
	}
	return openid
}
...
func (h *WechatKefuOpenidCacheBuildHandler) GetCacheData() (any, error) {
	return msql.Model(`wechat_kefu_openid_map`, lib_define.Postgres).Where(`openid`, h.Openid).Find()
}

func GetExternalUserInfo(openid string) (externalUserid, openKfid string) {
	result := make(msql.Params)
	err := lib_redis.GetCacheWithBuild(lib_define.Redis, &WechatKefuOpenidCacheBuildHandler{Openid: openid}, &result, time.Hour)
	if err != nil {
		logs.Error(err.Error())
		return
	}
